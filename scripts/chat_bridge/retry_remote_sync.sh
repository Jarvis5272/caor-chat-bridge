...
if ! python scripts/chat_bridge/update_chat_bridge_v3.py --locked-result results/final_result_cross_validation_20260711 --out chat_bridge --audit-out results/chat_bridge_v3_repair_20260711 >/dev/null; then
  echo "[$(date --iso-8601=seconds)] local generation failed"
  exit 1
fi
if ! python scripts/chat_bridge/validate_bridge_semantics_v3.py --bridge chat_bridge --out "$RESULT/BRIDGE_V3_SEMANTIC_VALIDATION.tsv" >/dev/null; then
