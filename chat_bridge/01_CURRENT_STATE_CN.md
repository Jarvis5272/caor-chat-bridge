...
最新结果目录 `results/baepc_feiw_real_data_sync_dryrun_20260704` 是当前 latest result；最新 label 是 `BAEPC_FEIW_REAL_SYNC_STOP_COMPLEXITY_TOO_HIGH`。

## clean IDS 数据状态

最新结果记录的数据状态来自 `results/baepc_feiw_real_data_sync_dryrun_20260704`。若报告中的 input scope 不完整，以源 artifact 为准，不在 bridge 中编造。
