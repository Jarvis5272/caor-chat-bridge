...
最新结果目录 `results/baepc_bounded_event_identity_refinement_20260704` 是当前 latest result；最新 label 是 `BAEPC_EVENT_IDENTITY_REFINEMENT_GO_TO_HAND_TOY`。

## clean IDS 数据状态

最新结果记录的数据状态来自 `results/baepc_bounded_event_identity_refinement_20260704`。若报告中的 input scope 不完整，以源 artifact 为准，不在 bridge 中编造。
