...
最新结果目录 `results/v1_full_stack_acceleration_20260708` 是当前 latest result；最新 label 是 `V1X_PARALLEL_FEASIBLE_BUT_NOT_EXACT_IDENTICAL`。

## clean IDS 数据状态

最新结果记录的数据状态来自 `results/v1_full_stack_acceleration_20260708`。若报告中的 input scope 不完整，以源 artifact 为准，不在 bridge 中编造。
...
当前 active track: `missing`。根据 latest gate，当前结论是 `V1X_PARALLEL_FEASIBLE_BUT_NOT_EXACT_IDENTICAL`。
...
V1X_PARALLEL_FEASIBLE_BUT_NOT_EXACT_IDENTICAL
